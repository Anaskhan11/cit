...
router.get('/', async (req, res) => {
  try {
    ...
    const todayScore = await db.getOne(
      'SELECT overall_score, clarity_score, tone_score, messages_sent FROM communication_scores WHERE user_id = ? AND date = CURDATE()',
      [req.userId]
    );

    // Get weekly progress
    const weeklyProgress = await db.getMany(
      'SELECT date, overall_score, messages_sent FROM communication_scores WHERE user_id = ? AND date >= DATE_SUB(CURDATE(), INTERVAL 7 DAY) ORDER BY date',
      [req.userId]
    );

    // Get notifications
    const notifications = await db.getMany(
      'SELECT id, type, title, content, is_read, created_at FROM notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT 10',
      [req.userId]
    );
...
router.get('/analytics', async (req, res) => {
  try {
    ...
    const messageActivity = await db.getMany(
      'SELECT DATE(m.created_at) as date, COUNT(*) as message_count, AVG(ma.clarity_score) as avg_clarity FROM messages m LEFT JOIN message_analysis ma ON m.id = ma.message_id WHERE m.sender_id = ? AND m.created_at >= DATE_SUB(CURDATE(), INTERVAL ? DAY) AND m.is_deleted = FALSE GROUP BY DATE(m.created_at) ORDER BY date',
      [req.userId, parseInt(period)]
    );

    // Tone analysis
    const toneAnalysis = await db.getMany(
      'SELECT ma.tone, COUNT(*) as count, AVG(ma.clarity_score) as avg_clarity FROM message_analysis ma JOIN messages m ON ma.message_id = m.id WHERE m.sender_id = ? AND m.created_at >= DATE_SUB(CURDATE(), INTERVAL ? DAY) GROUP BY ma.tone',
      [req.userId, parseInt(period)]
    );
    ...
    const responseTimeData = await db.getMany(
      'SELECT DATE(created_at) as date, AVG(response_time_seconds) as avg_response_time FROM communication_scores WHERE user_id = ? AND date >= DATE_SUB(CURDATE(), INTERVAL ? DAY) GROUP BY DATE(created_at) ORDER BY date',
      [req.userId, parseInt(period)]
    );

    // Improvement areas
    const improvementAreas = await db.getMany(
      'SELECT "Clarity" as area, AVG(clarity_score) as score FROM communication_scores WHERE user_id = ? AND date >= DATE_SUB(CURDATE(), INTERVAL ? DAY) UNION ALL SELECT "Tone" as area, AVG(tone_score) as score FROM communication_scores WHERE user_id = ? AND date >= DATE_SUB(CURDATE(), INTERVAL ? DAY) UNION ALL SELECT "Response Time" as area, AVG(response_time_score) as score FROM communication_scores WHERE user_id = ? AND date >= DATE_SUB(CURDATE(), INTERVAL ? DAY)',
...
router.get('/notifications', async (req, res) => {
  try {
    ...
    const notifications = await db.getMany(
      'SELECT id, type, title, content, related_id, is_read, read_at, created_at FROM notifications WHERE user_id = ? ORDER BY created_at DESC LIMIT ? OFFSET ?',
