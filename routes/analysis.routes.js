...
router.get('/message/:messageId', async (req, res) => {
  try {
    ...
    const analysis = await db.getOne(
      `SELECT ma.*, m.content, m.sender_id, m.group_id
  ...
  try {
    ...
    const stats = await db.getOne(
      `SELECT 
        ...
        COUNT(CASE WHEN ma.tone IN ('positive', 'very_positive') THEN 1 END) as positive_messages,
        COUNT(CASE WHEN ma.tone IN ('negative', 'very_negative') THEN 1 END) as negative_messages,
        COUNT(CASE WHEN ma.potential_misunderstanding = TRUE THEN 1 END) as misunderstandings,
        AVG(LENGTH(m.content)) as avg_message_length
       FROM messages m
       LEFT JOIN message_analysis ma ON m.id = ma.message_id
       WHERE m.sender_id = ? AND m.is_deleted = FALSE AND m.message_type = 'text'`,
      [req.userId]
    );

    // Get tone distribution
    const toneDistribution = await db.getMany(
      `SELECT ma.tone, COUNT(*) as count
       FROM message_analysis ma
       JOIN messages m ON ma.message_id = m.id
    ...
    const recentScores = await db.getMany(
      `SELECT date, overall_score, clarity_score, tone_score, response_time_score
...
router.get('/tips', async (req, res) => {
  try {
    ...
    const tips = await db.getMany(
      `SELECT ct.*
       FROM communication_tips ct
       WHERE ct.is_active = TRUE
       AND ct.id NOT IN (
         SELECT tip_id FROM user_tips WHERE user_id = ?
       )
       ORDER BY ct.priority ASC, RAND()
       LIMIT 5`,
      [req.userId]
    );
...
router.post('/tips/:tipId/feedback', async (req, res) => {
  try {
    ...
    await db.update(
      `UPDATE user_tips 
       SET helpful = ?, acknowledged = TRUE, acknowledged_at = NOW()
...
router.get('/misunderstandings', async (req, res) => {
  try {
    const misunderstandings = await db.getMany(
      `SELECT m.*, msg.content as message_content, g.name as group_name
       FROM misunderstandings m
       JOIN messages msg ON m.message_id = msg.id
       JOIN groups g ON m.group_id = g.id
       WHERE m.detected_by_user_id = ? OR msg.sender_id = ?
...
router.post('/misunderstanding', async (req, res) => {
  try {
    ...
    const misunderstandingId = await db.insert(
      `INSERT INTO misunderstandings 
       (group_id, message_id, detected_by_user_id, issue_type, description)
       VALUES (?, ?, ?, ?, ?)`,
  ...
  try {
    ...
    const leaderboard = await db.getMany(
      `SELECT 
        ...
        u.communication_score as score,
        COUNT(DISTINCT m.id) as message_count
       FROM users u
       LEFT JOIN messages m ON u.id = m.sender_id AND m.is_deleted = FALSE
       LEFT JOIN communication_scores cs ON u.id = cs.user_id ${dateFilter}
       WHERE u.is_active = TRUE
       GROUP BY u.id
       ORDER BY u.communication_score DESC
       LIMIT 20`
    );
    ...
    const userRank = await db.getOne(
      `SELECT COUNT(*) + 1 as rank
       FROM users
       WHERE communication_score > (
         SELECT communication_score FROM users WHERE id = ?
       ) AND is_active = TRUE`,
      [req.userId]
    );
